...
const retryOperation = async (operation: () => Promise<any>, maxRetries = 5, delay = 1000) => {
  let lastError;
...
const seed = async () => {
    ...
    for (const category of categories) {
        ...
        if (existingCategories.docs.length > 0) {
            ...
            parentCategory = existingCategories.docs[0];
        } else {
            parentCategory = await retryOperation(() => 
                payload.create({
                    collection: "categories",
                    data: {
                        name: category.name,
        ...
        for (const subCategory of category.subcategories || []) {
            ...
            if (existingSubcategories.docs.length > 0) {
                console.log(`Subcategory "${subCategory.name}" already exists, skipping creation...`);
            } else {
